...
test_that("Idaho DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Idaho DOE URL and verify HTTP 200")
})
...
test_that("Can download Idaho enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Idaho enrollment file", {
  skip_if_offline()
...
test_that("idschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- idschooldata::get_available_years()
  ...
  tryCatch({
    raw <- idschooldata:::get_raw_enr(test_year)
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- idschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- idschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- idschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- idschooldata::fetch_enr(test_year, tidy = TRUE)
