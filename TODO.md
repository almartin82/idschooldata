...
The pkgdown build is failing due to network timeouts when trying to
reach CRAN/Bioconductor servers to check package links.
...
- Network connectivity to `cloud.r-project.org` and `bioconductor.org`
  is failing
- This is an infrastructure/network issue, not a code problem
- The pkgdown build tries to check if the package is on
  CRAN/Bioconductor as part of building the home sidebar
